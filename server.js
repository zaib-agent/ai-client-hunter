...
function fallbackQualifyClients({ niche, location, service, sources }) {
  ...
  return sources
    .map((source) => {
      ...
      return {
        businessName: cleanBusinessName(title) || url.replace(/^https?:\/\//, "").split("/")[0],
        industry: niche,
...
function normalizePersonName(value) {
  ...
  if (/::|\||contact information|about us|home\s*page|official website|secretary of state/i.test(text)) {
    return "";
  }
  // Prefer real person-looking names (letters, limited length)
  if (!/^[A-Za-z][A-Za-z.'\- ]{0,60}$/.test(text)) return "";
...
function fallbackOutreachText(lead, sender = {}) {
  const businessName = cleanBusinessName(lead.businessName) || "your team";
...
async function generateOutreachText(lead, sender = {}) {
  ...
  const senderCompany = cleanString(sender.company);
  const recipientName = normalizePersonName(lead.contactName);
  const businessName = cleanBusinessName(lead.businessName) || "Business";
